...
% Sweep social_weight under the align2 circular-resource environment.
...
analysis_steps = 1:round(T / dt);

interaction_radius = 8.0;
observation_weight = 1.0;
obs_noise_sigma = 1;   % angular noise in radians

arena_radius = 20;
resource_distance = 120;  % kept for compatibility with endpoint modes
resource_angles = [pi/4, 3*pi/4];
resource_half_period = 100;
env_mode = 'cir';
resource_motion = env_mode;

% Used when env_mode = 'pos'. Kept here so this script can be edited easily.
resource_schedule = [
...
% Circular resource parameters.
resource_circle_radius = 120;
resource_circle_period = 200;
resource_circle_phase0 = pi/4;

save_results = true;
filename_save = ['save_sweep_social_weight_2d_Ringv4t_align2_cir_', ...
    datestr(now,30), '.mat'];
...
title(['cir resource, steps ' num2str(analysis_steps(1)) '-' ...
...
title(['repeat = ' num2str(repeat_num) ...
    ', circle period = ' num2str(resource_circle_period)])
