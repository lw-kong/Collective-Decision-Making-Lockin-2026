function [mu_last,sigma_last,po_last] = func_MeanField_TopK(...
...
%topK = 4;                  % Top-K among (k social + 1 personal), must satisfy 1<=K<=k+1
omega_o = 1.0;          % personal weight
%sw = 0.3;          % per-social-message weight
...
for t = 2:step_max
    ...
    p_sel_u  = binocdf(topK-1, k, p_tail_u);
    ...
    denom = omega_o*p_o + sw*K_s;   % mean-field denom approximation
