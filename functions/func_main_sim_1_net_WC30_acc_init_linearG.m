function [acc,u,est,m,g,G_ts] = func_main_sim_1_net_WC30_acc_init_linearG(...
    decay_rate, num_agents, T, dt, omega_g, noise_sigma,...
    network, diz_list, mw, init_m, Gmin, Gmax)
...
% G(t) function
G_ts(:) = Gmin + (Gmax-Gmin) * (1:len_time_steps)/len_time_steps;
...
m(:,1) = init_m * ones(num_agents,1); % _init
...
for t = 2:len_time_steps
    ...
    est(:,t) = ( decay_rate .* (network' * m(:,t-1)) + g(:,t) ) ./ ( decay_rate .* deg + 1 );
